# Graph Report - redact-all-the-things  (2026-06-17)

## Corpus Check
- 68 files · ~26,821 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 899 nodes · 1635 edges · 70 communities (61 shown, 9 thin omitted)
- Extraction: 67% EXTRACTED · 33% INFERRED · 0% AMBIGUOUS · INFERRED: 542 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3406340c`
...
1. `Job` - 62 edges
2. `PageManifest` - 56 edges
3. `SourceType` - 53 edges
4. `ControlPlaneService` - 50 edges
5. `JobStatus` - 47 edges
6. `RedactionRegion` - 47 edges
7. `FailureCode` - 44 edges
...
- `WorkerValidationConfig` --uses--> `ValidatedSource`  [INFERRED]
  tests/worker/test_validation.py → src/redact_api/worker.py
- `WorkerValidationConfig` --uses--> `WorkerValidationConfig`  [INFERRED]
  tests/worker/test_validation.py → src/redact_api/worker.py
- `WorkerValidationConfig` --uses--> `SourceValidationError`  [INFERRED]
  tests/worker/test_validation.py → src/redact_api/worker.py
...
Cohesion: 0.09
Nodes (61): AwsBatchSubmitter, test_permanent_failures_are_not_retryable(), test_retryable_failures_select_the_worker_mode(), DynamoJobRepository, Path, RecordingBatchSubmitter, AwsBatchSubmitter, DynamoJobRepository (+53 more)
...
Cohesion: 0.09
Nodes (62): BaseModel, FastAPI, LookupError, ContractModel, PageManifest, Normalized page-space rectangle selected for redaction., Versioned per-page suggestions and current redaction selections., Origin of a redaction region. (+54 more)
...
Cohesion: 0.10
Nodes (35): Any, datetime, FailureCode, build_service(), AWS Lambda entry point., Construct the production service from Lambda environment settings., analyze_source(), Validation-only analysis worker boundary. (+27 more)
...
Cohesion: 0.05
Nodes (36): Job Contracts Specification, Purpose, Requirement: Job status lifecycle, Requirement: Optimistic job updates, Requirement: Optimistic page-manifest persistence, Requirement: Ownership and access expiry, Requirement: Page manifest contract, Requirement: Persistent job record (+28 more)
...
Cohesion: 0.33
Nodes (5): Requirement: Permanent worker validation failures, Scenario: Persist unsupported content safely, Scenario: Persist validation failure safely, Scenario: Reject retry for permanent validation failures, ADDED Requirements
...
Cohesion: 0.50
Nodes (3): Analysis worker, Safe failures, Source validation
...
## Knowledge Gaps
- **401 isolated node(s):** `Collaboration Platform`, `Change Workflow`, `Git Conventions`, `Definition of Done`, `Engineering Guidance` (+396 more)
...
- **Why does `PageManifest` connect `Governance Specification` to `Technical Scope`, `Community 28`, `Community 6`?**
  _High betweenness centrality (0.008) - this node is a cross-community bridge._
- **Why does `ControlPlaneService` connect `Governance Specification` to `Technical Scope`, `Community 28`, `Community 6`?**
  _High betweenness centrality (0.006) - this node is a cross-community bridge._
- **Are the 55 inferred relationships involving `Job` (e.g. with `AwsBatchSubmitter` and `DynamoJobRepository`) actually correct?**
...
- **Are the 50 inferred relationships involving `SourceType` (e.g. with `AwsBatchSubmitter` and `DynamoJobRepository`) actually correct?**
  _`SourceType` has 50 INFERRED edges - model-reasoned connections that need verification._
